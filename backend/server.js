...
async function seedAdmin() {
  ...
  try {
    const adminExists = await User.findOne({ email: 'admin@ecotrack.com' });
    if (!adminExists) {
      await User.create({
        name: 'Admin',
      ...
      console.log('🛡️ Default admin seeded: admin@ecotrack.com / Admin@123');
    }
  } catch (err) {
    // Ignore duplicate key errors on restart
    if (err.code !== 11000) console.error('Admin seed error:', err);
  }
}
